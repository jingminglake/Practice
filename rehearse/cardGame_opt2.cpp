...
int numOfPlan(int n, int totalProfit, int totalCost, vector<int> &a, vector<int> &b) {
    ...
    vector<vector<int> > dp(totalCost + 1, vector<int>(totalProfit + 2, 0) );
    dp[0][0] = 1;
    for (int i = 1; i <= n; i++) {
        for (int j = totalCost; j >= b[i - 1]; j--) {
            for (int k = totalProfit + 1; k >= 0; k--) {
                int minP = min (totalProfit + 1, k + a[i - 1]);
                //if (j >= b[i - 1]) {
                    dp[j][minP] = (dp[j][minP] + dp[j - b[i - 1]][k]) % modNum;
                //}
            }
        }
    }
    ...
    for (int j = 0; j <= totalCost - 1; j++) {
        res = (res + dp[j][totalProfit + 1]) % modNum;
    }
    return res;
}
